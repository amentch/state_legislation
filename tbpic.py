...
def affinity_mat(F, vt):
    ...
    # print np.shape(nvec)
    N = spdiags(nvec,0,n,n).tocsr()
    # N = csr_matrix(ndiag)

    # d1 = np.dot(N, np.ones((np.shape(outer)[0],1)))
    # d2 = np.dot(F.T, d1)
    # d3 = np.dot(F, d2)
    d = tbdot.dot(N, tbdot.dot(F, tbdot.dot(F.T, tbdot.dot(N, np.ones((np.shape(F)[0],1))))))

    ddiag = spdiags(d.T.tolist()[0]).tocsr()
    Dinv = sp.sparse.linalg.inv(ddiag)
    
    # in1 = np.dot(N, vt)
    # in2 = np.dot(F.T, in1)
    # in3 = np.dot(F, in2)
    # in4 = np.dot(N, in3)
    in4m = csr_matrix(tbdot.dot(N, tbdot.dot(F, tbdot.dot(F.T, tbdot.dot(N, vt)))))
    # print np.shape(Dinv)
    # print np.shape(in4m)
    return tbdot.dot(Dinv, in4m.T)
